...
class BalanceCalculatorTest {
    ...
    fun `calculator produces expected balances for mixed expenses and settlements`() {
        val members = listOf(
            Member(id = 1, groupId = 9, name = "A", createdAt = 1),
            Member(id = 2, groupId = 9, name = "B", createdAt = 1),
            Member(id = 3, groupId = 9, name = "C", createdAt = 1)
        )
        val expenses = listOf(
            Expense(
                id = 10,
        ...
        assertEquals(400, balances.first { it.memberId == 1L }.netBalance)
        assertEquals(0, balances.first { it.memberId == 2L }.netBalance)
        assertEquals(-400, balances.first { it.memberId == 3L }.netBalance)

        val simplified = calculator.simplify(balances)
        assertEquals(1, simplified.size)
        assertEquals(3L, simplified.single().fromMemberId)
        assertEquals(1L, simplified.single().toMemberId)
    ...
    fun `settlement reduces outstanding balances`() {
        val members = listOf(
            Member(id = 1, groupId = 1, name = "Ali", createdAt = 1),
            Member(id = 2, groupId = 1, name = "Sara", createdAt = 1)
        )
        val expenses = listOf(
            Expense(
                id = 1,
        ...
        assertEquals(150, balances.first { it.memberId == 1L }.netBalance)
        assertEquals(-150, balances.first { it.memberId == 2L }.netBalance)
    }
}
